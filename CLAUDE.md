...
| Aktuelles Work Package | **CBP-WP-018** (`in-review`, **Phase B1 – Technical Implementation**) — zuletzt committed `4dec921` (Phase B0); **ADR-0013**/**D-052**/**D-053**; Evidence Schema **3.0 implementiert** (Runtime-Stand 3.0), Schema 1.0/2.0 fail-closed; **uncommitted, Commit-Autorität beim Human Maintainer** |
...
| **Synthetic Evidence Contract 3.0 MVP** | **lokaler Prototyp** (CBP-WP-018, ADR-0013, D-052/D-053, **uncommitted**) — synthetic-only, read-only, nicht persistent, fail-closed; Evidence-Schema **3.0** mit eingebetteten strukturierten Artefakten, **`security-control-form` + `control_id`**, Provenance-/Binding-Hashes inkl. Security-Contract-Bindung, deterministische **Invalid-/Stale-/Conflict-Erkennung** (ohne Uhr), **negative-evidence-only** (keine positive Gate-Erfüllung); Schema 1.0 **und 2.0** fail-closed; kein RT-2, keine Persistenz, keine Aktivierung; **558 Tests**, **nicht produktiv** |
