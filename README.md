...
| Aktuelles Work Package | **CBP-WP-022** (`in-review`, Phase B2B-P – New-target Initialization Plan and Safety Guard) — **D-057** Registrierung, **D-058** ADR-Gate (**`ADR_REQUIRED`**), **D-059** Architekturannahme und **D-060** Enforcement Contract (`accepted`, **A0**); **ADR-0014 `accepted`, A1** bleibt die bindende **Architekturauthority** — **Host-authoritative Enforcement mit deklarativem Zielmodell und read-only Runtime-Validierung**; **D-060** nimmt den Vertrag [KB_04_STAGE_1_ENFORCEMENT_CONTRACT.md](docs/security/KB_04_STAGE_1_ENFORCEMENT_CONTRACT.md) an (**`ADR_NOT_REQUIRED`**, nur solange er vollständig innerhalb ADR-0014 bleibt); B0 `committed` `e4caa14`, B1A `committed` `1a7696d`, B1B `committed` `b86a35f`, B1C `committed` `24de07e`, B2A `committed` `929d10b`, B2B-P uncommitted. **B2B-P ergänzt eine interne, read-only Initialisierungs*planung*** — strikter Neu-und-leer-Nachweis, Bestands- und Race-Guards, deterministisches Planmodell, drei aktivierte Contract-ReasonCodes; **120 neue Tests**, Gesamtsuite **1050 grün**. **Keine reale Initialisierung**: kein `apply_plan`, kein `mkdir`, kein `chmod`, kein `chown`, keine Dateisystemmutation — **keine operative KB-04-Evidenz**, `operationally_verified` bleibt `False`. **D-061** (`accepted`, **A0**) begrenzt **B2C** auf **Traceability und NT-Vorbereitung** — rein synthetisch, **keine operative Evidenz**, **keine Gateintegration**, kein Security-Control-Form-Artefakt. **D-062** (`accepted`, **A0**) präzisiert den tatsächlichen Abdeckungsstand der **45** Contract-Testkennungen: **37 synthetisch abgedeckt**, **zwei dokumentierte synthetische Coverage Gaps** (`KB04-T-P10`, `KB04-T-N25` — beide **Contract §10.3**, noch nicht implementiert) und **sechs B2D-real-only**. **Eine vollständige Traceability darf offene Lücken sichtbar machen** — eine dokumentierte Lücke ist **keine Bestehensaussage**, und *synthetisch abdeckbar ist nicht synthetisch abgedeckt*. **B2C-T-R setzt die vollständige 45er-Traceability um**: drei neue Testdateien bilden alle Kennungen mit dem Split **37 / 2 / 6** ab — **`KB04-T-P10`** und **`KB04-T-N25`** bleiben als **sichtbare Coverage Gaps** mit leerem `covered_by` stehen (Contract §10.3 nicht implementiert), die sechs real-only Fälle sind **nur deklarativ vorbereitet**. **152 neue Tests**, Gesamtsuite **1202 grün, 0 übersprungen**. **B2D-P legt den Profile-A-Integrationsplan vor** — **plan-only**, mit **VM-Referenzvariante**, verbindlichem **Snapshot-/Recovery-Gate** und gestuftem Phasenmodell. **B2D-GOV kanonisiert die Ausführungsvoraussetzungen** (**D-063**): **zwei neue Risiken** **R-35** und **R-36**, **kein Repository-Harness erforderlich** (`NO_HARNESS_REQUIRED`), und **jeder reale Lauf benötigt eine einmalige, nicht wiederverwendbare Freigabe**. **B2D-AUTH** (**D-064**) wählt dafür ein **leeres versioniertes Autorisierungstemplate** mit **20 Pre-run-Pflichtfeldern**; **ausgefüllte Kopien bleiben ausschließlich lokal** und gelangen niemals ins Repository. **B2D-ENV-GOV** (**D-065**) wählt als Referenzmodell eine **dedizierte, nicht produktive Profile-A-VM** mit lokaler Per-Target-Freigabe; **keine konkrete Infrastruktur wird im Repository benannt oder registriert**, und **keine reale Ausführung ist autorisiert**. **B2D-E-N07-GOV** (**D-066**) hält die **Instanziierungsgrenze der Rolle `retrieval`** fest: sie ist in der gegenwärtigen Profile-A-Ausbaustufe **nicht instanziiert** und wird **nicht** auf eine bestehende Dienstidentität abgebildet; **`KB04-T-N07` ist damit zurückgestellt, nicht gescheitert** — der Fall bleibt **`B2D_REAL_ONLY`** mit unveränderter Disposition, **kein Schreibversuch und kein Host-Precheck** ist autorisiert, und **`KB04-T-N14`** ist lediglich der **nächste mögliche Planungskandidat**. **Die vorbereitete Baseline und die Bindungen der bereits instanziierten Rollen bleiben gültig.** **B2D-E-N14-GOV** (**D-067**) klassifiziert anschließend die **Erhebungsbrücke** für `KB04-T-N14`: erlaubt ist ausschließlich eine **flüchtige, einmalige und nicht versionierte lokale Observation Bridge**, die eine reale `os.lstat`-Beobachtung in die **bereits vorhandenen** Produktionsstrukturen überführt und die **bereits exportierte** Validierungsfunktion aufruft — **kein Testhelper, kein Harness, keine CLI, kein Adapterexport, keine Produktionscodeänderung**. **Zwölf Überschreitungsgrenzen** lösen eine erneute Prüfung aus, der Befehlsentwurf trägt weiterhin **Platzhalter**, und **N14 bleibt nicht ausgeführt**. **Eine Autorisierung ist kein Nachweis.** **Keine reale Ausführung autorisiert, keine Gate- oder OD-37-Wirkung, reale Infrastruktur nicht autorisiert**. **Eine Vorbereitung ist kein Nachweis, und ein Plan ist keine Ausführung. KB-04 bleibt `DOCUMENTED ONLY`**. Zuvor **B2A: interner, read-only KB-04-Validator** (`core/core_brain/enforcement/`, sechs Module, 21 additive ReasonCodes, **206 neue Tests**, Gesamtsuite **930 grün**) — **rein intern, ausschließlich synthetisch nachgewiesen, keine CLI, keine Config, kein Deployment, keine Mutation**. **Keine reale Profil-A-Verifikation**: `operationally_verified` bleibt bei synthetischer Herkunft `False`. **KB-04 Enforcement Stage 1** bleibt `DOCUMENTED ONLY`, **keine Control- oder Gate-Hochstufung**, B2B/B2C/B2D **nicht freigegeben**. Zuvor abgeschlossen **CBP-WP-021** (`committed`, `complete`, 2026-08-03; **D-056**, `ADR_NOT_REQUIRED`; B0 `0cb4ea9`, B1/B2 `271acc7`); zuvor abgeschlossen **CBP-WP-020** (`committed`, `complete`; **D-055**; B0 `17057e2`, B1/B2 `9c6c0fb`, C `d6a1a3c`) |
| Nächster Schritt | **Wird durch den Human Maintainer und Nova separat bestimmt.** Das kanonische Security-Foundation-Testinventar ist reconciliiert: **32 Negativtests, 1 Positivtest, 33 Testfälle**; **0 von 32** und **0 von 1** ausgeführt. **Weiterhin keine Installation und keine Bereitstellung.** Die technische Umsetzung von KB-04 ist **nicht autorisiert**; B1 und B2 sind **nicht freigegeben**. **CBP-WP-023 ist nicht registriert** |
