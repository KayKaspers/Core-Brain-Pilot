...
| Geplante Work Packages | **CBP-WP-022** `in-review` (D-057 Registrierung, D-058 ADR-Gate, **D-059** Architekturannahme, **D-060** Enforcement Contract; **Phase B2A**, B0 `committed` `e4caa14`, B1A `committed` `1a7696d`, B1B `committed` `b86a35f`, B1C `committed` `24de07e`, B2A uncommitted; **ADR-0014 `accepted`/A1**, **Host-authoritative Enforcement mit deklarativem Zielmodell und read-only Runtime-Validierung**; Vertrag `accepted contract`; **B2 nicht autorisiert**). **CBP-WP-021** ist `committed` und `complete` (D-056; B0 `0cb4ea9`, B1/B2 `271acc7`). **CBP-WP-020** ist `committed` und `complete` (D-055; B0 `17057e2`, B1/B2 `9c6c0fb`, C `d6a1a3c`), B3 **ausgeschlossen**; CBP-WP-019 ist `committed` (`3c437f2`, D-054). **CBP-WP-023 nicht registriert, nicht begonnen, nicht autorisiert** — das KB-04-Paket ist lediglich als möglicher Kandidat vorgemerkt |
