...
| Aktuelles Work Package | **CBP-WP-022** (`in-review`, **Phase B2D-E-N14-GOV – D-067 N14 Ephemeral Observation Bridge Classification**) — **D-057** Registrierung, **D-058** ADR-Gate (**`ADR_REQUIRED`**), **D-059** Architekturannahme und **D-060** Enforcement Contract (alle `accepted`, **A0**, 2026-08-03); **ADR-0014 `accepted`, A1** — **Host-authoritative Enforcement mit deklarativem Zielmodell und read-only Runtime-Validierung**; Vertrag: [KB_04_STAGE_1_ENFORCEMENT_CONTRACT.md](../docs/security/KB_04_STAGE_1_ENFORCEMENT_CONTRACT.md); **KB-04 Enforcement Stage 1** = Stufe 1 der neunstufigen Durchsetzungsreihenfolge (**OS-Dateirechte**); B0 `committed` `e4caa14`, B1A `committed` `1a7696d`, B1B `committed` `b86a35f`, B1C `committed` `24de07e`, B2A `committed` `929d10b`, B2B-P `committed` `fff8227`, B2C.1 `committed` `38eb33f`, B2C.2 `committed` `117647f`, B2C-T-R `committed` `9cde9de`, B2D-P `committed` `b409d25`, B2D-GOV `committed` `7e8328a`, B2D-AUTH `committed` `1222ec0`, B2D-ENV-GOV `committed` `0cd21f5`, **B2D-E-N07-GOV uncommitted**; **D-061** (`accepted`, **A0**, `B2C_TRACEABILITY_AND_NT_PREPARATION_SELECTED`, `ADR_NOT_REQUIRED`) begrenzt **B2C** auf Traceability und NT-Vorbereitung; **D-062** (`accepted`, **A0**, `B2C_TRACEABILITY_COVERAGE_SPLIT_RECONCILED`, `ADR_NOT_REQUIRED`, 2026-08-04) setzt den kanonischen Abdeckungssplit über die **45** Contract-Testkennungen auf **37 `SYNTHETIC_COVERED` · 2 `SYNTHETIC_COVERAGE_GAP` (`KB04-T-P10`, `KB04-T-N25`, beide **Contract §10.3**) · 6 `B2D_REAL_ONLY`**; **B2C-T-R setzt die Traceability technisch um**: **drei neue Testdateien** (`tests/kb04_nt_fixtures.py`, `tests/test_kb04_contract_traceability.py`, `tests/test_kb04_nt_preparation.py`) mit den Traceability-Zählern **45 / 37 / 2 / 6**, **152 neuen Tests** und einer Testbasis von **1202 grün, 0 übersprungen**; **B2D-P ergänzt genau eine neue Plandatei** — [KB_04_PROFILE_A_INTEGRATION_PLAN.md](../docs/runtime/KB_04_PROFILE_A_INTEGRATION_PLAN.md), **plan-only**, ohne reale Infrastruktur; **B2D-GOV registriert D-063** (`accepted`, **A0**, `B2D_EXECUTION_PREREQUISITES_ESTABLISHED`, `ADR_NOT_REQUIRED`) sowie die beiden neuen kanonischen Risiken **R-35** und **R-36** (beide **hoch**, **offen**) und entscheidet die Harnessfrage mit **`NO_HARNESS_REQUIRED`**; **B2D-AUTH registriert D-064** (`accepted`, **A0**, `B2D_E_RUN_AUTHORIZATION_ARTIFACT_FORM_SELECTED`, `ADR_NOT_REQUIRED`, 2026-08-05) und legt **genau eine neue Datei** an — [KB_04_B2D_E_RUN_AUTHORIZATION_TEMPLATE.md](../docs/runtime/KB_04_B2D_E_RUN_AUTHORIZATION_TEMPLATE.md), ein **vollständig leeres** Pre-run-Autorisierungstemplate mit **20 Pflichtfeldern** `AUTH-01`…`AUTH-20`; **B2D-ENV-GOV registriert D-065** (`accepted`, **A0**, `B2D_REFERENCE_ENVIRONMENT_PREPARATION_MODEL_SELECTED`, `ADR_NOT_REQUIRED`, 2026-08-05) mit dem Modell **`DEDICATED_NON_PRODUCTION_PROFILE_A_VM_WITH_LOCAL_PER_TARGET_APPROVAL`** — **keine neue Datei**, keine konkrete Instanz; **B2D-E-N07-GOV registriert D-066** (`accepted`, **A0**, `PROFILE_A_RETRIEVAL_NOT_INSTANTIATED_N07_DEFERRED`, `ADR_NOT_REQUIRED`, 2026-08-05) — **Variante C**, **keine neue Datei**, **keine neue Risiko-ID**: die Rolle **`retrieval`** ist derzeit **nicht instanziiert**, **`KB04-T-N07` ist zurückgestellt** und bleibt **`B2D_REAL_ONLY`** mit unveränderter Traceability-Disposition, **`KB04-T-N14`** ist nur nächster Planungskandidat und **nicht autorisiert**; **B2D-E-N14-GOV registriert D-067** (`accepted`, **A0**, `N14_EPHEMERAL_LOCAL_OBSERVATION_BRIDGE_ALLOWED`, `ADR_NOT_REQUIRED`, 2026-08-05) — **Variante B**, **keine neue Datei**, **keine neue Risiko-ID**, **keine Produktionscodeänderung**: erlaubt ist ausschließlich eine **flüchtige, einmalige, run-spezifische und nicht versionierte lokale Observation Bridge** über **`os.lstat`** plus die **vorhandene exportierte** Funktion `validate_observation`; **kein H1-Testhelper, kein Harness, keine CLI, kein Adapterexport, keine Produktionsschnittstelle**, **kein `paths.check_path`**, **kein `Path.resolve()`**, **kein `realpath()`**; **zwölf Überschreitungsgrenzen** lösen eine erneute Decision- und ADR-Prüfung aus; **N14 bleibt nicht ausgeführt**, **NT-05 bleibt nicht ausgeführt**, der Befehlsentwurf trägt weiterhin **Platzhalter**, und **`B2D-E-N14-PREP-N1`** ist erst **nach Commit und Reconciliation** zulässig; Zähler **67/63/14**, **Risiken 35** mit Severity **20/14/1 unverändert**; Traceability **45/37/2/6 unverändert**, Tests **1202 unverändert**; **kein Produktionsmodul, keine §10.3-Implementierung, kein Evidence-Artefakt, kein Harness**; **technische B2C-T-Implementierung noch nicht begonnen und nicht autorisiert**; **Implementierungspfad** `core/core_brain/enforcement/` — sechs B2A-Module plus die beiden **Plan-only-Module** `filesystem_adapter.py` und `initialization.py`, insgesamt **24 additive `KB04-*`-ReasonCodes** in `errors.py`, **206 + 120 neue Tests**, Gesamtsuite **1050**; **keine CLI, keine Config, kein Deployment, keine Mutation, kein Apply**; Zähler unverändert **60/56/14**; KB-04 bleibt **`DOCUMENTED ONLY`**. **B2B, B2C und B2D nicht autorisiert.** Zuletzt abgeschlossen **CBP-WP-021** (`committed`, `complete`; B0 `0cb4ea9`, B1/B2 `271acc7`, C `0344774`). **CBP-WP-023 nicht registriert, nicht autorisiert** |
...
| Getroffene Entscheidungen | **67** (davon **63** mit A0) |
| Angenommene ADRs | **14** |
