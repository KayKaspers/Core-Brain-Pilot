...
| **Phase 1** | Proxmox-Referenzumgebung | **AUTHORIZED FOR PLANNING** — [Backlog](../docs/roadmap/PHASE_1_BACKLOG.md), [Foundation Plan](../docs/roadmap/PHASE_1_FOUNDATION_PLAN.md) F1–F5; **OD-26 geschlossen** (ADR-0007), Core-Repository `publication-capable by design` und **weiterhin privat**; **Mappingkonvention entschieden** (ADR-0008), Activation Gate `NOT EVALUATED`; **Sicherheitsgrundlage spezifiziert** (ADR-0009, 12 Kontrollen `DOCUMENTED ONLY`); **Runtime Skeleton lokal implementiert** (CBP-WP-012, `run` fail-closed, nicht produktionsbereit); **Ingest-Quarantäne-MVP lokal implementiert** (CBP-WP-013, ADR-0010, synthetic-only, keine Promotion, nicht produktiv); **Source-Registry-MVP lokal implementiert** (CBP-WP-014, ADR-0011, synthetic-only, deaktiviert, `activate` verweigert, nicht produktiv); **Source-Mapping-Draft-Validator-MVP lokal implementiert** (CBP-WP-015, ADR-0012, synthetic-only, read-only, 31-Feld-Vertrag, externe read-only Registry-Bindung, `activation-check` verweigert, nicht produktiv); **Mapping-Activation-Gate-Evaluator-MVP lokal implementiert** (CBP-WP-016, D-050, synthetic-only, read-only, nicht persistent, 20 Gate-Kriterien, Ausgabestatus nur `NOT_EVALUATED`/`BLOCKED`, `activation-evaluate` endet immer `BLOCKED`, nicht produktiv); **Synthetic-Evidence-Contract-3.0-MVP lokal implementiert** (CBP-WP-018, ADR-0013, D-052/D-053, synthetic-only, eingebettete Artefakte, `security-control-form` + `control_id`, Provenance-/Binding-Hashes inkl. Security-Contract-Bindung, deterministische Invalid-/Stale-/Conflict-Erkennung, negative-evidence-only, Schema 1.0 und 2.0 fail-closed, kein RT-2/Persistenz/Aktivierung, 558 Tests, nicht produktiv); **Security-Foundation-Readiness-Contract-MVP lokal implementiert** (CBP-WP-018, D-053, statischer reiner Vertrag 1.0, 12 dokumentierte / 7 runtime-scoped Controls / 11 `(criterion, control_id)`-Bindungen, nur synthetische Formprüfung, keine Security-Evaluation, kein Enforcement, keine Readiness-Aussage, nicht produktiv); CBP-WP-016 `committed` (`04c427c`, D-050), CBP-WP-017 `committed` (`d3168c4`, D-051); **CBP-WP-018 `committed`** (ADR-0013/D-052/D-053, Phase B0 `4dec921`, Phase B1 `5ee2e83`, Runtime-Stand Evidence Schema 3.0 mit Security Contract 1.0); **CBP-WP-019 `committed`** (`3c437f2`, D-054, Deployment Readiness Intake, DRC-Gesamtstatus **APPROVED BY HUMAN MAINTAINER** für Profil A seit 2026-07-29 bei 19 Prüfpunkten und **19 `ready` / 0 `blocked`**, rein dokumentarisch, Profil-A-Zielbild dokumentiert und **nicht bereitgestellt**); **CBP-WP-022 `in-review`** (D-057 Registrierung, D-058 ADR-Gate, **D-059** Architekturannahme und **D-060** Enforcement Contract (alle **A0**), KB-04 Enforcement Stage 1, **Phase B2C.1 – Synthetic Evidence Scope Decision**; B0 `committed` `e4caa14`, B1A `committed` `1a7696d`, B1B `committed` `b86a35f`, B1C `committed` `24de07e`; **ADR-0014 `accepted`, Autoritätsklasse A1** — **Host-authoritative Enforcement mit deklarativem Zielmodell und read-only Runtime-Validierung**; **Enforcement Contract `accepted contract`** (`ADR_NOT_REQUIRED` innerhalb ADR-0014); **interne read-only Plan- und Guard-API** `core/core_brain/enforcement/` implementiert (sechs B2A-Module plus `filesystem_adapter.py` und `initialization.py`, **24 additive ReasonCodes**, 206 + 120 neue Tests) — **keine CLI, keine Config, kein Deployment, keine Mutation, kein Apply, keine operative KB-04-Evidenz**; **D-061** begrenzt **B2C** auf eine **synthetische Traceability- und Fixturephase** — **kein Evidence-Producer, keine Gate-Eingabe**; **D-062** (`accepted`, **A0**, `B2C_TRACEABILITY_COVERAGE_SPLIT_RECONCILED`, `ADR_NOT_REQUIRED`) erlaubt der Traceability ausdrücklich die Disposition **`SYNTHETIC_COVERAGE_GAP`** neben `SYNTHETIC_COVERED` und `B2D_REAL_ONLY` — Split **37/2/6** über 45 Kennungen, Lücken **`KB04-T-P10`** und **`KB04-T-N25`**; **keine technische §10.3-Implementierung** (keine Schreibzeitvalidierung, keine atomare Ersetzungsprüfung, kein temporärer Schreibkontext), **keine CLI, keine Config, kein Deployment, kein Evidence-Producer**; **B2C-T-R implementiert die synthetische Traceability** — drei reine Test- und Fixturedateien, **45 Kennungen**, Split **37/2/6**, sechs deklarative real-only Preparation-Fixtures (`PREPARED_ONLY`/`NOT_EXECUTED`), **152 neue Tests**, Gesamtsuite **1202**; **kein Produktionsmodul**, kein Re-Export, keine Runtime-, CLI-, Deployment- oder Gatekopplung; **B2D-P ergänzt den Profile-A-Integrationsplan** — **plan-only**, geplant ausschließlich als **VM-Referenzvariante** (ein Container ist **nicht** als gleichwertig festgelegt), reale Identitätswerte ausschließlich im **lokalen Operator-Workspace** und niemals versioniert; **keine Configänderung, keine neuen öffentlichen Configschlüssel, kein Binding-Schema, kein Deployment, kein Harness, kein Script, keine Evidence-Integration**; **B2D-GOV entscheidet die Harnessfrage kanonisch mit `NO_HARNESS_REQUIRED`** (D-063) — **kein Harness erforderlich, keine CLI, keine Config, kein Deployment**: die reale Erhebung bleibt **lokale, hostseitige Operatorbeobachtung**, und der **bestehende Validator** verarbeitet sie mit Herkunft `OBSERVED`; **keine Evidence-Integration**, lokale Formen bleiben lokal-only; **B2D-AUTH ergänzt ein leeres Markdown-Autorisierungstemplate** (D-064, Variante **A1**) — **kein Script, keine CLI, keine Config, kein Deployment, keine maschinenlesbare Autorisierung**, weder YAML noch JSON noch TOML noch Environment-Semantik; **jede ausgefüllte Kopie bleibt lokal-only** im Operator-Workspace und gelangt niemals in einen Commit; **B2D-ENV-GOV wählt mit D-065 ein rein dokumentarisches Referenzumgebungsmodell** — **kein Deployment, keine konkrete VM, keine Config, keine lokalen Werte**: eine **dedizierte, nicht produktive Profile-A-VM** mit lokaler Per-Target-Freigabe, **im Repository weder benannt noch registriert**; **B2D-E-N07-GOV hält mit D-066 eine rein dokumentarische Instanziierungsgrenze fest** — **kein Deployment, keine Runtimekomponente, keine neue Identität, keine neue Gruppe, keine Config**: die Rolle **`retrieval`** ist derzeit **nicht instanziiert** und wird **nicht** auf eine bestehende Dienstidentität abgebildet, **`KB04-T-N07` bleibt `B2D_REAL_ONLY` und ist zurückgestellt**, **kein Schreibversuch und kein Host-Precheck** ist autorisiert; **B2D-E-N14-GOV klassifiziert mit D-067 ausschließlich eine flüchtige lokale Ausführungsklammer** — **kein Deployment, keine CLI, kein Adapterexport, kein versionierter Helper, keine Runtime- oder Deploymentkomponente, keine Produktionscodeänderung, keine Configänderung**: erlaubt ist allein eine **einmalige, nicht versionierte** Überführung einer realen `os.lstat`-Beobachtung in die **vorhandenen** Produktionsstrukturen samt Aufruf der **vorhandenen** Validierungsfunktion; **`KB04-T-N14` bleibt nicht ausgeführt** und **NT-05 nicht ausgeführt**; KB-04 bleibt `DOCUMENTED ONLY`, B2B-Apply/B2C-T/B2D **nicht autorisiert**, **keine reale Bereitstellung**, **kein RT-2**; B2C.2 `committed` `117647f`, B2C-T-R `committed` `9cde9de`, B2D-P `committed` `b409d25`, B2D-GOV `committed` `7e8328a`, B2D-AUTH `committed` `1222ec0`, B2D-ENV-GOV `committed` `0cd21f5`, B2D-E-N07-GOV `committed` `c4b0426`, B2D-E-N14-GOV uncommitted); **CBP-WP-021 `committed` und `complete`** (D-056, Canonical Security Test Inventory Reconciliation, abgeschlossen 2026-08-03; B0 `0cb4ea9`, B1/B2 `271acc7`; kanonisch **32 Negativtests / 1 Positivtest / 33 Testfälle**, **0 ausgeführt**; **NT-25 nicht aktiv** nach TT-5, **NT-32/NT-33 gültig**, die Zahl **31** überholt; Durchführung einschließlich der ausführbaren Profil-A-Artefakte abgeschlossen); **CBP-WP-020 `committed` und `complete`** (D-055, Controlled Profile-A Deployment Foundation; Zielzustand **Z1 erreicht** / Scope **S2 abgeschlossen** / RT-2-Grenze **P1 eingehalten**; B0 `17057e2`, B1/B2 `9c6c0fb`, C `d6a1a3c`; **Profil-A-Bundle als committetes Repository-Artefakt** — genau sieben Dateien unter `deployments/profile-a/`, zwei getrennte Service-Identitäten, fail-closed Compose- und Konfigurationsvorlagen, maschinenlesbare Mount-/Egress-/Secret-/Backup-/RT-2-Verträge, **deterministischer Offline-Validator** mit Exit 0 und `issues=0`, **166 Bundle-Validation-Tests**, Gesamtstand **724 Tests OK, 0 übersprungen**; Status ausschließlich *repository artifact implemented* / *offline validation passed* — **keine Bereitstellung, kein Containerstart, nicht operational**); **CBP-WP-023 nicht registriert, nicht autorisiert** |
